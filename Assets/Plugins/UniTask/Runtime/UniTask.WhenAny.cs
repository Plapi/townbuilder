...
namespace Cysharp.Threading.Tasks
{
    public partial struct UniTask
    {
        ...
        public static async UniTask<Func<CancellationToken, UniTask<T>>> WhenAny<T>(List<(UniTask task, Func<CancellationToken, UniTask<T>> func)> tasks)
        {
            var tasksArray = new UniTask[tasks.Count];
            for (var i = 0; i < tasks.Count; i++)
                tasksArray[i] = tasks[i].task;

            var result = await new UniTask<int>(new WhenAnyPromise(tasksArray, tasksArray.Length), 0);
            return tasks[result].func;
        }
        ...
        /// <summary>
        /// Returns the first race task that completes.
        /// </summary>
        /// <param name="tasks">List of race tasks and their processing tasks. Use UniTaskUtils.CreateRaceTasks to creat the list</param>
        /// <param name="raceCancellationTokenSource">Race CTS - Provide a local CTS that is canceled after one of tasks completes</param>
        /// <returns></returns>
        public static async UniTask<Func<CancellationToken, UniTask>> WhenFirst(List<(UniTask task, Func<CancellationToken, UniTask> func)> tasks, CancellationTokenSource raceCancellationTokenSource)
        {
            var tasksArray = new UniTask[tasks.Count];
            for (var i = 0; i < tasks.Count; i++)
                tasksArray[i] = tasks[i].task;
            ...
            var result = await new UniTask<int>(new WhenAnyPromise(tasksArray, tasksArray.Length), 0);
            raceCancellationTokenSource.Cancel();
            return tasks[result].func;
        }

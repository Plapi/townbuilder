...
namespace Cysharp.Threading.Tasks
{
    public static partial class UniTaskExtensions
    {
            ...
            [CallerMemberName] string memberName = "")
        {
            ...
            var sourceInfo = string.IsNullOrEmpty(context)
                ? $"{filePath}:{lineNumber} in {memberName}"
                : context;
            ...
            if (awaiter.IsCompleted)
            {
                ...
                {
                    var forgetEx = new ForgotTaskException(ex, sourceInfo);
                    UniTaskScheduler.PublishUnobservedTaskException(forgetEx);
                }
            }
            else
            {
            ...
            [CallerMemberName] string memberName = "")
        {
            ...
            {
                Forget(task, sourceInfo);
            }
            else
            {
                ForgetCoreWithCatch(task, exceptionHandler, handleExceptionOnMainThread, sourceInfo).Forget();
            }
        }

        static async UniTaskVoid ForgetCoreWithCatch(UniTask task,
            Action<Exception> exceptionHandler,
            ...
            [CallerMemberName] string memberName = "")
        {
            ...
            catch (Exception ex)
            {
                ...
                {
                    var forgetEx = new ForgotTaskException(ex2, sourceInfo);
                    UniTaskScheduler.PublishUnobservedTaskException(forgetEx);
                }
            }
        }
